...
$statusColors = ['pending'=>'yellow','processing'=>'blue','packed'=>'indigo','shipped'=>'purple','delivered'=>'green','cancelled'=>'red'];
...
$cancellable = in_array($order['status'], ['pending','processing','packed']);
?>
<div class="max-w-4xl mx-auto px-4 py-8" x-data="cancelOrder(<?= $order['id'] ?>)">
  ...
  <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" @click.self="showModal=false">
    <div class="w-full max-w-md bg-white dark:bg-[hsl(222,47%,10%)] border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-2xl">
      ...
      <p class="text-slate-500 dark:text-slate-400 text-sm mb-4">Are you sure you want to cancel order <strong class="text-slate-700 dark:text-slate-200"><?= clean($order['order_number']) ?></strong>? This action cannot be undone.</p>
      <div class="mb-4">
        ...
        <textarea x-model="reason" rows="2" placeholder="Why are you cancelling this order?"
                  class="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-[hsl(222,47%,13%)] border border-slate-200 dark:border-white/8 text-slate-700 dark:text-slate-300 text-sm placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-red-400 resize-none transition-colors"></textarea>
      </div>
      <div class="flex gap-3">
        <button @click="showModal=false" class="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 text-sm font-medium hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">Keep Order</button>
        <button @click="confirm()" :disabled="loading"
                class="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
          <span x-show="!loading">Cancel Order</span>
          <span x-show="loading" class="flex items-center gap-2"><span class="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> Cancelling...</span>
        </button>
      </div>
    </div>
  </div>
  ...
  <div class="flex items-center justify-between gap-3 mb-6 flex-wrap">
    ...
    <button @click="showModal=true"
            class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-red-600 dark:text-red-400 text-sm font-semibold hover:bg-red-100 dark:hover:bg-red-500/15 transition-all">
      <i class="fa-solid fa-xmark text-xs"></i> Cancel Order
    </button>
    <?php endif; ?>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="space-y-5">
      ...
      <div class="rounded-2xl bg-red-50 dark:bg-red-500/8 border border-red-200 dark:border-red-500/20 p-5">
        ...
        <?php endif; ?>
        <p class="text-sm text-red-600 dark:text-red-400">Reason: <?= clean($order['cancellation_reason']) ?></p>
      </div>
      <?php endif; ?>
    </div>
  </div>
