...
$pageTitle  = $product['name'] . ' | Buy Online – TT Electro Store';
$catLabel   = $product['category_name'] ? $product['category_name'].' – ' : '';
$brandLabel = $product['brand_name']    ? $product['brand_name'].' '       : '';
$pageDesc   = 'Buy ' . $brandLabel . $product['name'] . ' online from TT Electro Store – ' . $catLabel . 'electronics components store in Amravati, India. ' . rtrim(strip_tags($product['description'] ?? ''), '.')  . '. Fast delivery across India.';
...
<div class="max-w-7xl mx-auto px-4 py-8" x-data="productDetail()">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14">
    ...
    <!-- ── Image Gallery ──────────────────────────────────── -->
    <div x-data="productGallery(<?= htmlspecialchars(json_encode($images),ENT_QUOTES) ?>)"
         x-init="init()"
      ...
      <div x-show="lightbox" x-cloak
        ...
        <button @click="lightbox=false" class="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors z-10">
          <i class="fa-solid fa-xmark"></i>
        </button>
        <button @click="prev();$event.stopPropagation()" class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors z-10">
          <i class="fa-solid fa-chevron-left text-sm"></i>
        </button>
        <button @click="next();$event.stopPropagation()" class="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors z-10">
          <i class="fa-solid fa-chevron-right text-sm"></i>
        </button>
        <img :src="images[current]" alt="<?= clean($product['name']) ?>"
             class="max-w-full max-h-[90vh] object-contain rounded-xl shadow-2xl select-none">
        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
          <template x-for="(img,i) in images" :key="i">
            <button @click.stop="goTo(i)" class="w-2 h-2 rounded-full transition-all"
                    :class="current===i?'bg-white scale-125':'bg-white/30 hover:bg-white/60'"></button>
          </template>
        </div>
      </div>

      <!-- Main image with arrows -->
      <div class="relative rounded-2xl overflow-hidden bg-slate-50 dark:bg-[hsl(222,47%,10%)] border border-slate-200 dark:border-white/6 mb-3 aspect-square flex items-center justify-center shadow-sm group"
           @mouseenter="pauseAuto()" @mouseleave="resumeAuto()"
           @touchstart="touchStart($event)" @touchend="touchEnd($event)">

        <!-- Image with fade transition -->
        <template x-for="(img, i) in images" :key="i">
          <div class="absolute inset-0 flex items-center justify-center transition-opacity duration-400"
               :class="current===i ? 'opacity-100' : 'opacity-0 pointer-events-none'">
            <template x-if="img">
              <img :src="img" alt="<?= clean($product['name']) ?>"
        ...
        <!-- Prev/Next arrows (shown on hover, always on mobile) -->
        <template x-if="images.length > 1">
          <div>
            <button @click="prev()" class="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white/80 dark:bg-black/50 backdrop-blur-sm shadow-lg flex items-center justify-center text-slate-700 dark:text-white hover:bg-white dark:hover:bg-black/70 transition-all opacity-0 group-hover:opacity-100 focus:opacity-100 z-10 sm:flex" aria-label="Previous">
              <i class="fa-solid fa-chevron-left text-xs"></i>
            </button>
            <button @click="next()" class="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white/80 dark:bg-black/50 backdrop-blur-sm shadow-lg flex items-center justify-center text-slate-700 dark:text-white hover:bg-white dark:hover:bg-black/70 transition-all opacity-0 group-hover:opacity-100 focus:opacity-100 z-10 sm:flex" aria-label="Next">
        ...
        <template x-if="images.length > 1">
          <div class="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
            <template x-for="(img,i) in images" :key="i">
              <button @click="goTo(i)" class="w-1.5 h-1.5 rounded-full transition-all"
                      :class="current===i?'bg-white w-4':'bg-white/50 hover:bg-white/80'"></button>
            </template>
          </div>
        </template>

        <!-- Expand icon hint -->
        <div class="absolute top-3 right-3 w-7 h-7 rounded-lg bg-black/30 backdrop-blur-sm flex items-center justify-center text-white/70 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
          <i class="fa-solid fa-expand text-[10px]"></i>
        </div>

        <!-- Auto-play indicator bar -->
        <template x-if="images.length > 1 && autoRunning">
          <div class="absolute bottom-0 left-0 h-0.5 bg-blue-500/60 transition-none z-20" :style="'width:'+progress+'%'"></div>
        </template>
      </div>
      ...
      <template x-if="images.length > 1">
        <div class="flex gap-2 overflow-x-auto pb-1 scrollbar-thin">
          <template x-for="(img, i) in images" :key="i">
            <button @click="goTo(i)"
                    :class="current===i?'border-blue-500 dark:border-blue-400 shadow-md shadow-blue-500/20 scale-105':'border-slate-200 dark:border-white/10 hover:border-blue-400 dark:hover:border-blue-500/40'"
                    class="flex-shrink-0 w-16 h-16 rounded-xl border-2 overflow-hidden transition-all duration-200 bg-slate-50 dark:bg-[hsl(222,47%,10%)]">
              <template x-if="img">
                <img :src="img" class="w-full h-full object-cover select-none" draggable="false">
              </template>
              <template x-if="!img">
...
<?php
/* ── build SEO data ──────────────────────────────────────── */
$_seoImages = array_values(array_filter($images));
if(empty($_seoImages)) $_seoImages = [APP_URL.'/assets/logo.png'];

$_seoKeywords = array_unique(array_filter([
  $product['name'],
  $product['brand_name'] ?? null,
  $product['category_name'] ?? null,
  'TT Electro Store', 'TT Electronics', 'TT Electro',
  'electronics store', 'electronics components', 'buy electronics online india',
  'electronics store amravati', 'electronics components amravati',
]));

$_seoReviews = [];
foreach(array_slice($reviews,0,5) as $rv) {
  if(empty($rv['body'])) continue;
  $_seoReviews[] = [
    '@type'        => 'Review',
    'reviewRating' => ['@type'=>'Rating','ratingValue'=>(int)$rv['rating'],'bestRating'=>5],
    'name'         => clean($rv['title'] ?? 'Customer Review'),
    'reviewBody'   => clean(substr($rv['body'],0,300)),
    'author'       => ['@type'=>'Person','name'=>clean($rv['user_name']??'Verified Buyer')],
    'datePublished'=> date('Y-m-d', strtotime($rv['created_at'])),
    'publisher'    => ['@type'=>'Organization','name'=>'TT Electro Store'],
  ];
}
$_breadcrumbs = [
...
if($product['category_name'] && $product['category_id']) {
  $_breadcrumbs[] = ['@type'=>'ListItem','position'=>3,'name'=>clean($product['category_name']),'item'=>APP_URL.'/products?category_id='.$product['category_id']];
  $_breadcrumbs[] = ['@type'=>'ListItem','position'=>4,'name'=>clean($product['name']),'item'=>APP_URL.'/products/'.$product['id']];
} else {
  $_breadcrumbs[] = ['@type'=>'ListItem','position'=>3,'name'=>clean($product['name']),'item'=>APP_URL.'/products/'.$product['id']];
}
...
$_jsonld = [
  ...
  '@graph'   => [
    [
      '@type'       => 'Product',
      '@id'         => APP_URL . '/products/' . $product['id'] . '#product',
      'name'        => clean($product['name']),
      'description' => clean(strip_tags($product['description'] ?? $product['name'])),
      'image'       => count($_seoImages) === 1 ? $_seoImages[0] : $_seoImages,
      'sku'         => 'TTE-' . $product['id'],
      'mpn'         => 'TTE-' . $product['id'],
      'keywords'    => implode(', ', $_seoKeywords),
      'category'    => $product['category_name'] ?? 'Electronics Components',
      'brand'       => [
        '@type' => 'Brand',
        'name'  => clean($product['brand_name'] ?? 'TT Electro Store'),
      ],
      'seller' => [
        '@type' => 'Organization',
      ...
      'offers' => [
        ...
        'seller' => ['@type'=>'Organization','@id'=>APP_URL.'/#organization','name'=>'TT Electro Store'],
      ],
      'aggregateRating' => count($reviews) > 0 ? [
      ...
      'review' => !empty($_seoReviews) ? $_seoReviews : null,
    ],
    [
      '@type'           => 'BreadcrumbList',
      '@id'             => APP_URL . '/products/' . $product['id'] . '#breadcrumb',
      'itemListElement' => $_breadcrumbs,
    ],
  ],
];
// strip nulls for cleaner output
$_jsonld['@graph'][0] = array_filter($_jsonld['@graph'][0], fn($v) => $v !== null);
?>
<script type="application/ld+json">
<?= json_encode($_jsonld, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>
</script>
  ...
  return {
    ...
    startAuto() {
      ...
      this._prog = setInterval(() => {
        this.progress = Math.min(100, this.progress + (tick / interval) * 100);
      }, tick);
    ...
    stopAuto() {
      ...
      this._timer = null; this._prog = null;
      this.autoRunning = false;
      this.progress = 0;
    },

    pauseAuto()  { if (this._timer) { clearTimeout(this._timer); clearInterval(this._prog); this.autoRunning = false; } },
    resumeAuto() { if (this.images.length > 1 && !this._timer) this.startAuto(); },
